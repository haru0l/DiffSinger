...
class VarianceBinarizer(BaseBinarizer):
    ...
    def process_item(self, item_name, meta_data, binarization_args):
        ...
        if hparams['predict_energy']:
            ...
            if energy is None:
                energy = get_energy_librosa(waveform, length, hop_size=hparams['hop_size'], win_size=hparams['win_size'], domain='db').astype(np.float32)
        ...
        if hparams['predict_tension']:
            ...
            if tension is None:
                tension = get_tension_base_harmonic_logit(dec_waveform, None, None, length=length)
